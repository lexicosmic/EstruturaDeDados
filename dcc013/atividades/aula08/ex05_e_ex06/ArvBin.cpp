...
void ArvBin::preOrdem()
{
    ...
    if (p != NULL)
    {
        ...
        while (!pilha.vazia())
        {
            ...
            cout << q->getInfo() << " ";
            if (q->getDir() != NULL)
                pilha.empilha(q->getDir());
            if (q->getEsq() != NULL)
                pilha.empilha(q->getEsq());
        };
    }
}
...
void ArvBin::percursoLargura()
{
    ...
    {
        pilha.empilha(p);      // coloca nó raiz na fila
        while (!pilha.vazia()) // há nós na fila?
        {
            q = pilha.desempilha(); // tira nó da fila
            cout << q->getInfo() << endl;
            if (q->getEsq() != NULL)
                pilha.empilha(q->getEsq());
            if (q->getDir() != NULL)
