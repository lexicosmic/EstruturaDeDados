...
{
    int tamNova = max + l2->max;
    ListaOrdCont *lNova;
    lNova = new ListaOrdCont(tamNova);

    int i = 0;
    int j = 0;
    int k = 0;

    while (i < tamNova && j < n && k < l2->n)
    {
        if (vet[j] < l2->vet[k])
        {
            lNova->vet[i] = vet[j];
    ...
    else if (k < l2->n && i < tamNova)
        for (int l = k; l < l2->n; l++)
        {
            lNova->vet[i] = l2->vet[l];
            lNova->n++;
            i++;
        }
